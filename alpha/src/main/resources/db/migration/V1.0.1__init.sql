...
CREATE TABLE `s_login_log` (
  ...
  `logout_time` datetime DEFAULT NULL COMMENT '推出时间',
  `account_id` bigint DEFAULT NULL COMMENT '用户id',
  `session_id` bigint DEFAULT NULL COMMENT 'session_id',
  `address_ip` varchar(12) DEFAULT NULL COMMENT 'ip地址',
  `operator_type` varchar(1) DEFAULT NULL COMMENT '操作类型 (登录、退出)',
  `execute_status` bit(1) DEFAULT NULL COMMENT '执行结果',
  `executeErrorMSg` varchar(255) DEFAULT NULL COMMENT '执行失败原因',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
-- ----------------------------
...
CREATE TABLE `s_operator_log` (
  `id` varchar(19) NOT NULL COMMENT 'id',
  `operator_start_time` datetime NOT NULL COMMENT '执行开始时间',
  `operator_end_time` datetime DEFAULT NULL COMMENT '执行结束时间',
  `url` varchar(255) DEFAULT NULL COMMENT '访问url',
  `method_type` varchar(8) DEFAULT NULL COMMENT '方法类型',
